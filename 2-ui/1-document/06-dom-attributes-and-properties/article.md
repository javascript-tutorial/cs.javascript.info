...
Když prohlížeč načítá stránku, „načte“ (jiným slovem: „parsuje“) HTML a vygeneruje z něj DOM objekty. U elementových uzlů se pro většinu standardních HTML atributů automaticky vytvoří vlastnosti DOM objektů.
...
Značky v HTML mohou obsahovat atributy. Když prohlížeč parsuje HTML a vytváří ze značek DOM objekty, rozpoznává *standardní* atributy a vytváří pro ně DOM vlastnosti.
...
Tyto metody pracují přesně s tím, co je uvedeno v HTML.
...
<body>
  ...
  <script>
    ...
    alert( elem.outerHTML ); // (3), podíváme se, zda je atribut v HTML (je)
...
1. `getAttribute('Téma')` -- zde je první písmeno velké, zatímco v HTML jsou všechna písmena malá. Na tom však nezáleží: v názvech atributů se velká a malá písmena nerozlišují.
...
4. Kolekce `attributes` je iterovatelná a obsahuje všechny atributy elementu (standardní i nestandardní) jako objekty s vlastnostmi `name` a `value`.
...
Když píšeme HTML, používáme spoustu standardních atributů. Ale co ty nestandardní, naše vlastní? Nejprve se podíváme, zda jsou vůbec užitečné. K čemu jsou?
...
- Chceme načíst hodnotu „tak, jak je uvedena“ v HTML. Hodnota DOM vlastnosti se může lišit, například vlastnost `href` je vždy úplné URL, ale my můžeme chtít získat „původní“ hodnotu.
